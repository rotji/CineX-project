  ...
  return (
    <div className={`fixed inset-0 z-50 flex items-center justify-center ${className}`}>
      ...
      <div className="relative bg-white rounded-xl shadow-2xl max-w-lg w-full mx-4 max-h-[90vh] overflow-hidden border-2 border-gray-300">
        ...
        {/* Content */}
        <div className="px-6 py-4 max-h-96 overflow-y-auto bg-gray-100" onScroll={handleScrollCheck}>
          {/* Transaction Description */}
          <div className="mb-4 bg-white p-4 rounded-lg border-2 border-blue-200 shadow-sm">
            <p className="text-gray-900 font-semibold text-base">{transactionData.description}</p>
          </div>
          ...
          {/* Transaction Details */}
          <div className="space-y-3 bg-white p-5 rounded-lg border-2 border-gray-300 shadow-sm">
            <h3 className="font-bold text-gray-900 mb-4 text-lg border-b-2 border-gray-200 pb-2">💳 Transaction Details</h3>
            {/* Amount */}
            {transactionData.amount && (
              <div className="flex justify-between items-center py-4 border-2 border-blue-200 bg-blue-50 px-4 rounded-lg">
            ...
            {transactionData.recipient && (
              <div className="flex justify-between items-center py-4 border-2 border-green-200 bg-green-50 px-4 rounded-lg">
                <span className="text-sm font-bold text-gray-900">👤 Recipient</span>
                <div className="text-right">
                  {transactionData.recipientName && (
                    <p className="font-bold text-gray-900 text-base">{transactionData.recipientName}</p>
                  )}
                  <p className="font-mono text-sm text-gray-800 bg-white px-3 py-1 rounded border-2 border-gray-300">
            ...
            {transactionData.estimatedFees && (
              <div className="flex justify-between items-center py-4 border-2 border-orange-200 bg-orange-50 px-4 rounded-lg">
                <span className="text-sm font-bold text-gray-900">⚡ Estimated Fees</span>
            ...
            {transactionData.riskLevel && (
              <div className="flex justify-between items-center py-4 border-2 border-purple-200 bg-purple-50 px-4 rounded-lg">
                <span className="text-sm font-bold text-gray-900">🛡️ Risk Level</span>
            ...
            {transactionData.metadata && Object.keys(transactionData.metadata).length > 0 && (
              <div className="py-4 bg-indigo-50 px-4 rounded-lg border-2 border-indigo-200">
                <div className="text-base font-bold text-indigo-900 mb-3 border-b border-indigo-300 pb-2">📋 Additional Details</div>
                <div className="space-y-3">
                  {Object.entries(transactionData.metadata).map(([key, value]) => (
                    <div key={key} className="flex justify-between text-sm bg-white p-3 rounded-lg border-2 border-gray-200 shadow-sm">
                      <span className="text-gray-800 font-bold capitalize">{key.replace(/([A-Z])/g, ' $1')}</span>
          ...
          {transactionData.warnings && transactionData.warnings.length > 0 && (
            <div className="mt-4 p-5 bg-red-100 border-4 border-red-400 rounded-lg shadow-lg">
              <div className="flex items-center mb-4">
                ...
                <span className="text-lg font-bold text-red-900">⚠️ IMPORTANT WARNINGS</span>
              </div>
              <ul className="space-y-3">
                {transactionData.warnings.map((warning, index) => (
                  <li key={index} className="flex items-start bg-white p-4 rounded-lg border-2 border-red-300 shadow-sm">
                    <span className="text-red-700 font-bold mr-3 text-lg">⚠️</span>
                    <span className="font-bold text-red-900 text-base">{warning}</span>
                  </li>
                ))}
              </ul>
            </div>
          ...
          {transactionData.contractCall && (
            <div className="mt-4">
              <button
                ...
                className="flex items-center text-base font-bold text-blue-700 hover:text-blue-900 bg-blue-50 px-4 py-2 rounded-lg border-2 border-blue-200 hover:bg-blue-100 transition-colors"
              >
              ...
              {isAdvancedOpen && (
                <div className="mt-4 p-5 bg-gray-900 rounded-lg text-base border-4 border-gray-600 shadow-lg">
                  <div className="space-y-4">
                    <div className="bg-gray-800 p-3 rounded-lg border-2 border-gray-600">
                      <span className="text-white font-bold">📄 Contract:</span>
        ...
        {/* Footer */}
        <div className="px-6 py-5 bg-gray-200 border-t-4 border-gray-400 flex items-center justify-between">
          <div className="text-base text-gray-900 font-bold bg-yellow-100 px-4 py-2 rounded-lg border-2 border-yellow-400">
          ...
          <div className="flex space-x-3">
            <button
              ...
              className="px-8 py-4 text-base font-bold text-gray-900 bg-white border-4 border-gray-500 rounded-lg hover:bg-gray-100 hover:border-gray-700 disabled:opacity-50 disabled:cursor-not-allowed transition-all shadow-lg"
            >
              ❌ Cancel
            </button>
            <button
              onClick={onConfirm}
              disabled={isProcessing}
              className="px-8 py-4 text-base font-bold text-white bg-blue-600 border-4 border-blue-800 rounded-lg hover:bg-blue-700 hover:border-blue-900 disabled:opacity-50 disabled:cursor-not-allowed transition-all shadow-lg flex items-center space-x-2"
            >
              {isProcessing && (
